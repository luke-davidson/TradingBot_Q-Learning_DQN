...
from agents.dqn5_agent import DQN5Agent, evaluate_dqn5_agent, train_dqn5_agent
...
def avg_last(observation, index, n):
...
class StocksEnvWithFeatureVectors(StocksEnv):
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        position_onehot = [0] * len(position_values)
        position_onehot[observation.position_history[-1] - np.min(position_values)] = 1  # most recent position

        return np.array([
            avg_last(observation.history, 0, 200),  # 200 day moving average
            avg_last(observation.history, 0, 50),  # 50 day moving average
            *position_onehot  # most recent position
        ])
...
def main():
    ...
    # create experiment
    train_returns, train_loss, train_profits, validation_profits, agent = train_dqn5_agent(env, train_parameters)
                                                                                           # model_file='dqn5_best.pt')
    ...
    plt.grid()
    plt.savefig(f'dqn5_profits_train_{name}')
    plt.clf()
    ...
    plt.grid()
    plt.savefig(f'dqn5_profits_validation_{name}')
    ...
    ExperimentResult(
        ...
        algorithm=f'dqn5_{name}'
    ).to_file()

    agent.to_file(f'{name}_model.pt')

    best_agent = DQN5Agent(train_parameters)
    best_agent.load_model('dqn5_best.pt')

    env.set_mode(Mode.Test)
    test_profits = evaluate_dqn5_agent(env=env, agent=best_agent, params={
        'episodes': 200,
    ...
    plt.legend()
    plt.savefig(f'dqn5_profits_test_{name}')
